#Leonardo Roman. 
#io.s code is being modified to print a random number from 1 to 9.
#In this modification polling, I got rid off the polling state and
#modified chracter write in line 110 in order to print a number when 
#an interrupt occurs(key stroke). Also an additional loop was added
#inside the l3 label in order to create a random number as ascii code
#increments from 48 to 58 which is ascii code for 0 to 9.
.data
...
RandomNumber:				
	li $s6, 48					#Random numeber generator fro 0 to 9
l3:	
	addi $s6, $s6, 1			#Increment 1 to create random number
	beq $s6, 58, RandomNumber	#loop back to RandomNumber generator
